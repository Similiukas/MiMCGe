...
% pereiti per notes is messenger

words: 632/981/1932/4345/5891/8032/9962
...
\subsection{Our Goals}\label{sub:goals}
In this project, we are going to focus on MiMC, which is constructed of the simplified round function of the Knudsen-Nyberg cipher from 1995\cite{KNCipher}. The core component of this algorithm is the use of \(f(x)=x^3\) APN (almost perfect non-linear) function \textcolor{red}{add reference to what it is or explain it or both}. What we are going to do is \textit{build new ciphers based on the structure of MiMC by trying different polynomials to better understand how cryptographic algorithms are proven to be secure and then test cryptographic properties and see whether our newly built cipher is efficient.}
...
As discussed in chapter \ref{sub:prng} about statistical test suites, a sample space needs to be provided. Here it is discussed more broadly how these inputs were generated.
...
Recall, that the main goal of the project was: to built new ciphers, argue their security and test cryptographic properties (chapter \ref{sub:goals}). And in fact, this goal was achieved. First, it was necessary to prove that in the chosen field $\operatorname{GF}(2^n)$ it is possible to find any function which will be a cipher by the definition \ref{def:cipher}. For that, permutation polynomials of any exponent were chosen and this gave the \textit{MiMCGe} cipher (chapter \ref{sub:mimcge}). Then the task was to prove that these class of ciphers are indeed secure.
